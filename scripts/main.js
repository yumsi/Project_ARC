...
function fill_selection_cases(cases) {
    ...
    $(document).ready(function () {
        for (let i = 0; i < data.length; i++) {
            $('#dynamic-select').append('<option>' + data[i] + '</option>');
        }
    })
}
...
async function color_network(selected) {
    ...
    for (const [key, value] of Object.entries(data)) {
        if (key === selected) {
            ...
            for (const [idx, values] of Object.entries(data[key])) {
                for (const [file_id, val] of Object.entries(data[key][idx])) {
                    if (file_id.endsWith("_Primary_Tumor")) {
                        const tumor_div = document.getElementById("stringEmbedded2").getElementsByClassName("notselectable")[0];
                        const nodes = tumor_div.getElementById("nodes")
                        for (let element of nodes.getElementsByTagName("g")) {
                            for (let entry of val) {
                                if (element.getAttribute("data-safe_div_label") === entry['gene']) {
                                    let color_class = element.getElementsByClassName("nwbubblecoloredcircle")
                                    color_class[0].setAttribute("fill", entry['colour'])
                                }
                            }
                            document.getElementById("right-side").innerHTML = "<h3>" + file_id + "</h3><br>";
                        }

                    } else if (file_id.endsWith("_Solid_Tissue_Normal")) {
                        const normal_div = document.getElementById("stringEmbedded").getElementsByClassName("notselectable")[0];
                        const normal_nodes = normal_div.getElementById("nodes")
                        for (let element of normal_nodes.getElementsByTagName("g")) {
                            for (let entry of val) {
                                if (element.getAttribute("data-safe_div_label") === entry['gene']) {
                                    let color_class = element.getElementsByClassName("nwbubblecoloredcircle")
                                    color_class[0].setAttribute("fill", entry['colour'])
                            ...
                            document.getElementById("left-side").innerHTML = "<h3>" + file_id + "</h3><br>";
    ...
    // let retrieve_selected_case = data.some(i => i.name.includes(selected_case));
    // filter(cls => selected_case.includes(cls.className))
    // console.log(retrieve_selected_case)
...
async function send_request_to_string() {
    await retrieve_selected_cases()
...
function getSTRING(root_url, params) {
    ...
    var stringDiv = document.getElementById('stringEmbedded'),
        stringDiv2 = document.getElementById("stringEmbedded2");
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == XMLHttpRequest.DONE) {
            ...
            stringDiv2.innerHTML = xhr.responseText;
